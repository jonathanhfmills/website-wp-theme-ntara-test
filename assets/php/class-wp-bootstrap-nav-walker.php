...
if ( ! class_exists( 'WP_Bootstrap_Navwalker' ) ) {
	...
	class WP_Bootstrap_Navwalker extends Walker_Nav_Menu {
		...
		public function start_lvl( &$output, $depth = 0, $args = array() ) {
			...
			$classes = array( 'submenu-menu', 'dropdown-menu', 'border-top', 'border-bottom' );
			/**
			...
			if ( end( $matches[2] ) ) {
				...
				$idvalue = 'id ="submenu-' . esc_attr( end( $matches[2] ) ) . '"';

			}
			$output .= "{$n}{$indent}<ul $idvalue $class_names $labelledby role=\"menu\">{$n}";
		}
		...
		public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
			...
			if ( isset( $args->has_children ) && $args->has_children && 0 === $depth && $args->depth > 1 ) {
				$atts['href'] = '#submenu-menu-item-dropdown-' . $item->ID;
